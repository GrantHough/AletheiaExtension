...
chrome.runtime.onMessage.addListener( // Listener for popup in top right
    function(request, sender, sendResponse) {
        ...
        if (request.action == "bias") {
            const response = articleBias(sendResponse);
        }
        if (request.action == "summarize") {
            const response = articleSummary(sendResponse);
        }
        return true;
    }
  );
...
function articleBias(sendResponse) {
...
function articleSummary(sendResponse) {
    const getActiveUrl = (tabid, changeInfo, tab) => {
        const url = changeInfo.url;
        // url is likely to be empty, and filter chrome:// and about:// URLs
        if (!url || ['chrome://', 'about://'].some(p => url.startsWith(p))) return;
